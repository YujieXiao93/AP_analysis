...
Binwidth=50;
BinL=0:Binwidth:950-Binwidth;%这里需要根据protocol step范围改一下
BinR=BinL+Binwidth;
...
%                 pause
        ...
        Wantidx=find((spkFreq>=Num*2&spkFreq<=500)&(StimAmp(:)<1550));%想要分析的sweep的频率限定
        ...
        if ~isempty(Wantidx)
            ...
            for jj=1:length(Wantidx)
                ...
                [spkwaveform,ttspanspk,HW, AHP, Threshold, Amp,max_slope,min_slope,...
                 ...
                 max_slopeidx,min_slopeidx] = AP_Statistic_HERE(data,spkTime{Wantidx(jj)},tspanV,1);%AP_Statistic_HERE里的pretime和postime会影响ttspanspk   
...
%                 pause
            ...
            xlswrite('Results_Statistic_temp.xls',spkParamName,['Param_',GroupName{kk},'_Cell_',FilenameRecord{kk}{k,1}],'B1')
            xlswrite('Results_Statistic_temp.xls',[HWtemp',...
                AHPtemp',...
                Thresholdtemp',...
                Amptemp',...
                max_slopetemp',...
                min_slopetemp'],['Param_',GroupName{kk},'_Cell_',FilenameRecord{kk}{k,1}],'B2')
...
for k=1:length(spkParamName)
    ...
    for kk=1:length(Fidx)
        data{kk}=spkparamRecord{kk}{k}(thAP,:);
        N=sum(~isnan(data{kk}));%统计7±3的AP串中第几个AP的波形参数
        Mean=nanmean(data{kk});
        Sem=nanstd(data{kk})./sqrt(N);
        figure(5)
